...
// Track job progress for live polling simulation
let jobProgressTracker = {
  transcribe: 45,
  analyze: 0,
  transcribeTimer: 0,
  analyzeTimer: 0,
...
export function setupMockApi() {
  ...
  initialized = true;
  console.log("[MockAPI] 🧪 Mock API interceptor active");
  ...
  api.interceptors.response.use(async (response) => {
    ...
    if (url === "/api/auth/register" && method === "POST") {
      return {
        ...response, status: 201,
        data: { status: "success", data: { user: { ...mockUser, id: generateId("user") } }, token: "mock_token_demo" },
      };
    }
    if (url === "/api/auth/login" && method === "POST") {
      return { ...response, status: 200, data: { status: "success", data: { user: mockUser }, token: "mock_token_demo" } };
    }
    if (url === "/api/auth/me" && method === "GET") {
      return { ...response, status: 200, data: { status: "success", data: { user: mockUser } } };
    }

    // ── Videos ──
    if (url === "/api/videos" && method === "POST") {
      const isMultipart = config.headers?.["Content-Type"]?.toString().includes("multipart");
      let source_url = "";
      let title = null;
      if (isMultipart) {
    ...
    if (url === "/api/videos" && method === "GET") {
      return { ...response, status: 200, data: { status: "success", data: { videos: mockVideos, total: mockVideos.length } } };
    }

    if (url.startsWith("/api/videos/") && method === "GET") {
      const videoId = url.replace("/api/videos/", "").split("/")[0];
      const video = mockVideos.find((v) => v.id === videoId);
      if (!video) return { ...response, status: 404, data: { status: "error", message: "Video not found" } };

      // Simulate job progress for live polling
      const videoJobs = mockJobs.filter((j) => j.video_id === videoId);
      if (videoId === "vid_003") {
        // Progress the transcribe job
        ...
        if (transcribeJob) {
          ...
          if (transcribeJob.progress >= 100) {
            ...
            if (analyzeJob) {
              analyzeJob.status = "processing";
              analyzeJob.progress = 0;
            }
          }
        }
        // Progress the analyze job
        const analyzeJob = videoJobs.find((j) => j.id === "job_006");
        if (analyzeJob && analyzeJob.status === "processing") {
          ...
          if (analyzeJob.progress >= 100) {
            ...
            // Mark video as ready when all jobs done
            const allDone = videoJobs.every((j) => j.status === "completed");
            if (allDone) {
        ...
        data: {
          ...
          data: {
            video: {
              ...video,
              transcripts: [],
              highlights: mockHighlights[videoId] || [],
              jobs: videoJobs,
            },
          },
        },
    ...
    if (url.match(/^\/api\/videos\/[\w-]+\/analyze$/) && method === "POST") {
      return { ...response, status: 202, data: { status: "accepted", message: "Analysis started", data: { job_id: generateId("job") } } };
    }

    if (url.match(/^\/api\/videos\/[\w-]+\/highlights$/) && method === "GET") {
      const videoId = url.split("/")[4]; // /api/videos/:id/highlights
      const highlights = mockHighlights[videoId] || [];
      return { ...response, status: 200, data: { status: "success", data: { video_id: videoId, highlights, total: highlights.length } } };
    }

    if (url.match(/^\/api\/videos\/[\w-]+$/) && method === "DELETE") {
      return { ...response, status: 200, data: { status: "success", data: { message: "Video deleted successfully" } } };
    }
    ...
    if (url === "/api/clips/generate" && method === "POST") {
      ...
      return {
        ...
        data: {
          status: "success",
          data: {
            clip: { ...clip, aspect_ratio: body.aspect_ratio || "16:9", quality: body.quality || "720p" },
          },
        },
      };
    }
    ...
    if (url === "/api/clips" && method === "GET") {
      ...
      return { ...response, status: 200, data: { status: "success", data: { clips, total: clips.length } } };
    }

    if (url.match(/^\/api\/clips\/[\w-]+$/) && method === "GET") {
      const clipId = url.split("/").pop()!;
      const clip = mockClips.find((c) => c.id === clipId);
      if (!clip) return { ...response, status: 404, data: { status: "error", message: "Clip not found" } };
      return { ...response, status: 200, data: { status: "success", data: { clip } } };
    }

    if (url.match(/^\/api\/clips\/[\w-]+$/) && method === "DELETE") {
      return { ...response, status: 200, data: { status: "success", data: { message: "Clip deleted successfully" } } };
    }

    if (url.match(/^\/api\/download\/[\w-]+$/) && method === "GET") {
      return { ...response, status: 200, data: new Blob(["mock video content"], { type: "video/mp4" }) };
    }
    ...
    if (url === "/api/admin/users" && method === "GET") {
      ...
      return {
        ...response, status: 200,
        data: { status: "success", data: { users: mockUsers, total: mockUsers.length } },
      };
    }
    ...
    if (url === "/api/admin/storage" && method === "GET") {
      return {
        ...
        data: {
          status: "success",
          data: {
            uploads_size: 2_456_000_000, // ~2.4 GB
            generated_clips_size: 1_234_000_000, // ~1.2 GB
            total_size: 3_690_000_000, // ~3.7 GB
            free_space: 45_000_000_000, // ~45 GB
            total_space: 100_000_000_000, // ~100 GB
            uploads_path: "/app/uploads",
            generated_clips_path: "/app/generated-clips",
          },
        },
      };
    }
      ...
      return {
        ...
        data: {
          ...
          data: {
            ...
            total_clips: 156,
            total_users: 12,
            avg_processing_time: 34.5, // seconds
            videos_today: 5,
            clips_today: 18,
            videos_this_week: 23,
            clips_this_week: 72,
